 ...
 * 
 * @author Alex Blewitt &lt;<I><A
 *         href="mailto:Alex.Blewitt@ioshq.com">Alex.Blewitt@ioshq.com</A></I>&gt;
 * @version 1.0
 */
public class BeanPropertyUtil {
	 ...
	 * {@link java.beans.PropertyDescriptor}, and then the accessor method is
	 * invoked dynamically using {@link java.lang.reflect.Method}.
	 * 
	 ...
	 *             if the <I>name</I> does not exist in <I>instance</I> or
	 *             there is an exception dynamically invoking the accessor
	 *             method .
	 */
	...
	/**
	 * Obtain the read method for property <em>name</em> associated with
	 * <em>instance</em>. Once the method is located, it is stored in
	 * {@link #map} using a format <code>fully.qualified.Class#name</code> so
	 * that it is obtained quicker on subsequent requests. As the map is a
	 * {@link java.util.WeakHashMap} then memory requests may cause parts of
	 * this cache to be lost.
	 * 
	 * @param name
	 *            the property name
	 * @param instance
	 ...
	 */
	public static Method getReadMethod(String property, Object instance)
			throws IllegalArgumentException, IntrospectionException {
		String id = instance.getClass() + "#" + property;
		...
		if (read == null)
			throw new IllegalArgumentException(
					"Cannot find instance with property '" + property + "'");
		return read;
	}

	public static Object setProperty(String property, Object bean, Object value)
			throws IllegalArgumentException {
		Method read = null;
		try {
			read = getReadMethod(property, bean);
			Method write = getWriteMethod(bean.getClass(), property, read.getReturnType());
			if (write == null)
				throw new IllegalArgumentException(
						"Cannot find write instance with property '" + property + "'");
			return write.invoke(bean, new Object[]{value});
		} catch (Exception e) {
			throw new IllegalArgumentException("Problem writing property '"
					+ property + "' expected " + read.getReturnType() + " and found " + value.getClass().getName() + ": " + e.getMessage());
		}
	}
